...
long pSyncBcast[_SHMEM_BCAST_SYNC_SIZE];
...
int main(int argc, char *argv[])
{
    ...
    for ( t = 0; t < _SHMEM_BCAST_SYNC_SIZE; t += 1) pSyncBcast[t] = _SHMEM_SYNC_VALUE;
	...
	t_start = TIME();
	shmem_broadcast32(buffer, buffer, size, 0, 0, 0, numprocs, pSyncBcast);
	t_stop = TIME();
